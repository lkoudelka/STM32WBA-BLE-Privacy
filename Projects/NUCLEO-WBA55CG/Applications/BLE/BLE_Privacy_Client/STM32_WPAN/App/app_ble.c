...
typedef struct
{
  ...
  uint8_t a_deviceServerAddrType;
...
static UTIL_TIMER_Object_t stop_scan_timer;
static bool allow_connection = true;
static bool allow_RPA_resolution = true;
/* USER CODE END PV */
...
static void stop_scan_timer_cb (void *);
static void stop_scan_timer_task(void);
/* USER CODE END PFP */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask( 1u << CFG_TASK_SCAN_TERMINATE_ID, UTIL_SEQ_RFU, stop_scan_timer_task);
  UTIL_TIMER_Create(&stop_scan_timer, 1000, UTIL_TIMER_ONESHOT, stop_scan_timer_cb, NULL);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      Bonded_Device_Entry_t Bonded_Device_Entry[10];
      List_Entry_t List_Entry[10];
      aci_gap_get_bonded_devices( &Num_of_Addresses, Bonded_Device_Entry);
      LOG_INFO_APP("==>> aci_gap_get_bonded_devices : success\n\r");
      LOG_INFO_APP("     - Num of addresses:   0x%x\n", Num_of_Addresses);
      for (uint8_t index = 0; index < Num_of_Addresses; index ++)
      {
        LOG_INFO_APP("     - addresse %d type:    0x%x\n     - addresse %d val:    0x%02x%02x%02x%02x%02x%02x\n\r", index, Bonded_Device_Entry[index].Address_Type, index, 
                                                                                          Bonded_Device_Entry[index].Address[5],
                                                                                          Bonded_Device_Entry[index].Address[4],
                                                                                          Bonded_Device_Entry[index].Address[3],
                                                                                          Bonded_Device_Entry[index].Address[2],
                                                                                          Bonded_Device_Entry[index].Address[1],
                                                                                          Bonded_Device_Entry[index].Address[0]);
      

        List_Entry[index].Address_Type = Bonded_Device_Entry[index].Address_Type;
        List_Entry[index].Address[5] = Bonded_Device_Entry[index].Address[5];
        List_Entry[index].Address[4] = Bonded_Device_Entry[index].Address[4];
        List_Entry[index].Address[3] = Bonded_Device_Entry[index].Address[3];
        List_Entry[index].Address[2] = Bonded_Device_Entry[index].Address[2];
        List_Entry[index].Address[1] = Bonded_Device_Entry[index].Address[1];
        List_Entry[index].Address[0] = Bonded_Device_Entry[index].Address[0];
            
      }
      
      if (Num_of_Addresses > 0)
      {
        ret = aci_gap_add_devices_to_list( Num_of_Addresses, &List_Entry[0], 0x1);
        if (ret != BLE_STATUS_SUCCESS)
        {
          LOG_INFO_APP("==>> aci_gap_add_devices_to_list : fail : 0x%x\n\r", ret);
        }
        else
        {
          LOG_INFO_APP("==>> aci_gap_add_devices_to_list : success\n\r");
        }
      }
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        default:
        {
          ...
          LOG_INFO_APP(" ==> Unknown HCI LE Meta event code : %02X\n\r", p_meta_evt->subevent);
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
        {
          ...
          if ((p_gap_proc_complete->Procedure_Code == GAP_GENERAL_DISCOVERY_PROC) &&
              ...
              (bleAppContext.deviceServerFound == 1)&&
        ...
        /* USER CODE BEGIN ECODE_1 */
          case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
            {
              aci_gap_pairing_complete_event_rp0 *pairing_complete = (aci_gap_pairing_complete_event_rp0*) p_blecore_evt->data;
              ...
              ret = hci_disconnect( pairing_complete->Connection_Handle,
                                       0x13);
              if (ret != BLE_STATUS_SUCCESS)
              {
                LOG_INFO_APP("==>> hci_disconnect : fail : 0x%x\n\r", ret);
              }
              else
              {
                LOG_INFO_APP("==>> hci_disconnect : success\n\r");
              }              
            }
            break;
        ...
        case ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE:
          {
            aci_gap_numeric_comparison_value_event_rp0 *numeric_comparaison_value  = (aci_gap_numeric_comparison_value_event_rp0*) p_blecore_evt->data;

            LOG_INFO_APP(">>== ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE\n");

            LOG_INFO_APP("     - Connection handle : 0x%04x\n     - numeric_value = %ld\n     - Hex_value = %lx\n", numeric_comparaison_value->Connection_Handle, numeric_comparaison_value->Numeric_Value, numeric_comparaison_value->Numeric_Value);

            aci_gap_numeric_comparison_value_confirm_yesno(numeric_comparaison_value->Connection_Handle, 0x1);  
          }
            break;
        ...
        default:
        {
          ...
          LOG_INFO_APP(" ==> Unknown HCI vendor specific event code : %02X\n\r", p_blecore_evt->ecode);
    ...
    {
      hci_encryption_change_event_rp0 *encryption_change;
      encryption_change = (hci_encryption_change_event_rp0 *) p_event_pckt->data;
      LOG_INFO_APP(">>== HCI_ENCRYPTION_CHANGE_EVT_CODE\n");
      LOG_INFO_APP("     - Connection Handle:   0x%02X\n     - Status :    0x%02X\n     - Enable :    0x%02X\n",
                   encryption_change->Connection_Handle,
                   encryption_change->Status,
                   encryption_change->Encryption_Enabled);
    ...
    default:
    {
      ...
      LOG_INFO_APP(" ==> Unknown HCI event code : %02X\n\r", p_event_pckt->evt);
...
void APP_BLE_Procedure_Gap_Central(ProcGapCentralId_t ProcGapCentralId)
{
  ...
  switch(ProcGapCentralId)
  {
    case PROC_GAP_CENTRAL_SCAN_START:
    {
      status = aci_gap_start_general_discovery_proc(paramA, paramB, GAP_RESOLVABLE_PRIVATE_ADDR, 0x0);
      if (status != BLE_STATUS_SUCCESS)
      {
      ...
      UTIL_TIMER_Start(&stop_scan_timer);
      break;
    }/* PROC_GAP_CENTRAL_SCAN_START */
    case PROC_GAP_CENTRAL_SCAN_TERMINATE:
    {
      ...
      UTIL_TIMER_Stop(&stop_scan_timer);
      break;
    }/* PROC_GAP_CENTRAL_SCAN_TERMINATE */
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  const uint8_t a_BLE_CfgIrValue[16] = {p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0],p_bd_addr[0]};
  LOG_INFO_APP("  IRK: ");
  for (int8_t i = 15; i >= 0; i--)
  {
    LOG_INFO_APP("%02X:",a_BLE_CfgIrValue[i]);
  }
  LOG_INFO_APP("\n");
  ...
  const uint8_t a_BLE_CfgErValue[16] = {p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1],p_bd_addr[1]};
  LOG_INFO_APP("  ERK: ");
  for (int8_t i = 15; i>= 0; i--)
  {
    LOG_INFO_APP("%02X", a_BLE_CfgErValue[i]);
  }
  LOG_INFO_APP("\n");
...
static uint8_t analyse_adv_report(hci_le_advertising_report_event_rp0 *p_adv_report)
{
  ...
  uint16_t i;
  tBleStatus ret;
  uint8_t peer_RPA_adress[6];
  bool known_bd_address = false;

  found_status = 0;
  if ((adv_type = p_adv_report->Advertising_Report[0].Event_Type) != 0x00)
  {
    return 0;
  }
  ...
  LOG_INFO_APP("New Adv report -> Even Type : 0x%02x  - Address Type : 0x%02x  - Address : 0x%02X:%02X:%02X:%02X:%02X:%02X  - Length Data  : %d\n\r",
            adv_type,             
            p_adv_report->Advertising_Report[0].Address_Type,
            p_adv_report->Advertising_Report[0].Address[5],
            p_adv_report->Advertising_Report[0].Address[4],
            p_adv_report->Advertising_Report[0].Address[3],
            p_adv_report->Advertising_Report[0].Address[2],
            p_adv_report->Advertising_Report[0].Address[1],
            p_adv_report->Advertising_Report[0].Address[0],
            adv_data_size);
  p_adv_data = (uint8_t*)(&p_adv_report->Advertising_Report[0].Length_Data) + 1;
  
  if (adv_data_size > 0)
    LOG_INFO_APP("    --> adv data :");
  for (i = 0; i < adv_data_size; i++)
  {
    LOG_INFO_APP("%02x", p_adv_data[i]);
  }
  LOG_INFO_APP("\n\r");
  
  if (allow_RPA_resolution == true)
  {
    ret = hci_le_read_peer_resolvable_address( 0x00,
                                              &(p_adv_report->Advertising_Report[0].Address[0]),
                                              &(peer_RPA_adress[0]));
    if (ret == BLE_STATUS_SUCCESS)
    {
      known_bd_address = true;
    }
  }
    ...
    while(i < adv_data_size)
    {
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
        {
          ...
          {
            bleAppContext.a_deviceServerAddrType = p_adv_report->Advertising_Report[0].Address_Type;
            ...
            bleAppContext.deviceServerFound = 0x1;
            
            LOG_INFO_APP("  P2P server\
                         \n     - Even Type    : 0x%02x\
                         \n     - Address Type : 0x%02x\
                         \n     - bd addr      : 0x%02X:%02X:%02X:%02X:%02X:%02X\
                         \n     - Length Data  : %d\n",
                            p_adv_report->Advertising_Report[0].Event_Type,
                            p_adv_report->Advertising_Report[0].Address_Type,
                            ...
                            p_adv_report->Advertising_Report[0].Length_Data);
            if (adv_data_size > 0)
              LOG_INFO_APP("     - Adv Data     : ");
            for (uint8_t index = 0; index < adv_data_size; index++)
            {
              LOG_INFO_APP("%02x", p_adv_data[index]);
            }
            if (adv_data_size > 0)
              LOG_INFO_APP("\n\n\r");
            ...
            if (known_bd_address == true)
            {
              LOG_INFO_APP("Address RPA  : 0x%02X:%02X:%02X:%02X:%02X:%02X\n\r",
                  peer_RPA_adress[5],
                  peer_RPA_adress[4],
                  peer_RPA_adress[3],
                  peer_RPA_adress[2],
                  peer_RPA_adress[1],
                  peer_RPA_adress[0]);    
              LOG_INFO_APP("    --> device already bonded\n\n");
              found_status = 0x0;
            } 
          }
          break;
...
static void Connect_Request(void)
{
  ...
  LOG_INFO_APP("Create connection to P2Pserver\n");

  result = aci_gap_create_connection(SCAN_INT_MS(500u), SCAN_WIN_MS(500u),
                                     bleAppContext.a_deviceServerAddrType,
                                     &bleAppContext.a_deviceServerBdAddr[0],
...
{
  allow_connection = APPE_ButtonIsLongPressed(B1);
...
void APPE_Button2Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    ret = aci_gap_add_devices_to_list(0, NULL, 0x01);
    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("==>> clear resolving list - Fail, result: 0x%02X \n", ret);
    }
    else
    {
      LOG_INFO_APP("==>> clear resolving list - Success\n");
    }    
  }
  else
  {
...
{
  allow_RPA_resolution = !allow_RPA_resolution;
  tBleStatus ret = hci_le_set_address_resolution_enable(allow_RPA_resolution);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("==>> hci_le_set_address_resolution_enable - Fail, result: 0x%02X \n", ret);
    LOG_INFO_APP("==>> This command can't be used when advertising or scanning is enabled and when Create connection command is outstanding\n");
  }
  else
  {
    if (allow_RPA_resolution == true)
    {
      LOG_INFO_APP("==>> Successfully enabled hci_le_set_address_resolution_enable \n");
    }
    else
    {
      LOG_INFO_APP("==>> Successfully disabled hci_le_set_address_resolution_enable \n");
    }
  }
  return;
}
...
void stop_scan_timer_cb(void * arg)
{
  UTIL_SEQ_SetTask(1U << CFG_TASK_SCAN_TERMINATE_ID, CFG_SEQ_PRIO_0);
  return;
}
