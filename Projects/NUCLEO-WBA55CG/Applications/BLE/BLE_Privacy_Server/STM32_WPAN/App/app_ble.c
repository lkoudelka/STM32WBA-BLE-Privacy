...
#define ADV_TIMEOUT_MS                 (10 * 1000)
#define INITIAL_ADV_TIMEOUT            (10)
/* USER CODE END PD */
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      ret = aci_gap_get_bonded_devices( &Num_of_Addresses, Bonded_Device_Entry);
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("==>> aci_gap_get_bonded_devices : fail : 0x%x\n\r", ret);
      }
      else
      {
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          ret = aci_gap_peripheral_security_req(p_enhanced_conn_complete->Connection_Handle);
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("==>> aci_gap_peripheral_security_req : fail : 0x%x\n\r", ret);
          }
          else
          {
            LOG_INFO_APP("==>> aci_gap_peripheral_security_req : success\n\r");
          }
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        {			
          LOG_INFO_APP(">>== ACI_GAP_PERIPHERAL_SECURITY_INITIATED_VSEVT_CODE\n");
          LOG_INFO_APP("==>> Peripheral Security Request is successfully sent to the Central\n");
          break;
        }
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_PRINT_BD_ADDR:
    {
      uint8_t Num_of_Addresses;
      Bonded_Device_Entry_t Bonded_Device_Entry[10];
      uint8_t Local_Resolvable_Addres[6];
      ...
      else
      {
        for (uint8_t index = 0; index < Num_of_Addresses; index ++)
        {
          ...
          ret = hci_le_read_local_resolvable_address(Bonded_Device_Entry[index].Address_Type, Bonded_Device_Entry[index].Address, Local_Resolvable_Addres);
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("==>> hci_le_read_local_resolvable_address : fail : 0x%x\n\r", ret);
          }
          else
          {
            LOG_INFO_APP("==>> hci_le_read_local_resolvable_address : success\n");
            if (CFG_BD_ADDRESS_TYPE == GAP_STATIC_RANDOM_ADDR)
              LOG_INFO_APP("  -> Static Random address:");
            else
              LOG_INFO_APP("  -> Public bd address:");
            LOG_INFO_APP(" 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
                                                                ...
                                                                (uint8_t)(p_bd_addr[0]));
            LOG_INFO_APP("  -> Resolvable Private Address (RPA) : 0x%02X:%02X:%02X:%02X:%02X:%02X\n\r",
                                                                Local_Resolvable_Addres[5],
                                                                Local_Resolvable_Addres[4],
                                                                Local_Resolvable_Addres[3],
                                                                Local_Resolvable_Addres[2],
                                                                Local_Resolvable_Addres[1],
                                                                Local_Resolvable_Addres[0]);
          }
        }
      }
      break;
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
   ...
   */
    p_bd_addr[0] = (uint32_t)((company_id & 0x000000FF) << 24) | (uint32_t)(device_id << 16) | (uint32_t)(udn & 0x0000FF00) | (uint32_t)(udn & 0x000000FF);
    p_bd_addr[1] = (uint32_t)((company_id & 0x00FF0000) >> 16) | (uint32_t)((company_id & 0x0000FF00) >> 8);
  }
	else
	{
  ...
  const uint8_t a_BLE_CfgIrValue[16] = {(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0]),(uint8_t)(p_bd_addr[0])};
  LOG_INFO_APP("  IRK = ");
  ...
  const uint8_t a_BLE_CfgErValue[16] = {(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8),(uint8_t)(p_bd_addr[0] >> 8)};
  LOG_INFO_APP("  ERK = ");
  for (int8_t i = 15; i>= 0; i--)
  {
  ...
  ret = hci_le_set_resolvable_private_address_timeout(0x000A);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : hci_le_set_resolvable_private_address_timeout command, result: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: hci_le_set_resolvable_private_address_timeout command | Set to 10 sec\n");
  }
...
void APPE_Button1Action(void)
{
  if (bleAppContext.Device_Connection_Status == APP_BLE_IDLE)
  {
    ...
    fill_advData(&a_AdvData[0], sizeof(a_AdvData), (uint8_t*) p_bd_addr);
    APP_BLE_Procedure_Gap_Peripheral(PROC_GAP_PERIPH_ADVERTISE_START_FAST);
    APP_BLE_Procedure_Gap_Peripheral(PROC_GAP_PERIPH_PRINT_BD_ADDR);
    UTIL_TIMER_StartWithPeriod(&bleAppContext.Advertising_mgr_timer_Id, ADV_TIMEOUT_MS);
  }
  else
  {
...
void APPE_Button2Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_SERVER)
  {
    ...
    ret = aci_gap_add_devices_to_list( 0, NULL, 0x1);
    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("==>> clear resolving list : fail : 0x%x\n\r", ret);
    }
    else
    {
